...
if [ "$1" = 'start' ]; then
  ...
  collectContainerLogs() {
    ...
    curl -s --no-buffer -XGET --unix-socket /var/run/docker.sock "http:/containers/$CONTAINER/logs?stderr=1&stdout=1&tail=1&follow=1" | sed '/^ *$/d' | sed "s;^;[$CONTAINER_NAME] ;" > $NAMED_PIPE
